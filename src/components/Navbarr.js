  ...
  return (
    <div className='flex w-full z-50 h-[60px] text-white fixed justify-between items-center bg-gray-800'>
         
         {
          ...
          <ul ref={sidebarRef} className={`fixed w-2/4  flex flex-col z-50 sm:hidden h-dvh top-0 bg-gray-800 items-center justify-around  transition-all ease-in-out duration-300 ${nav ? "translate-x-0,overflow-hidden,bg-fixed":"translate-x-[-900px]"}`}>
            ...
            {
                navItems.map(item =>(
                  <a href={`#${item.title}`} onClick={navHandler} className='w-full h-10 flex cursor-pointer items-center hover:shadow-lg hover:border-t hover:border-b justify-center' key={item.id}>
                    <li>{item.title}</li>
                  </a>
                ))
              }
        ...
        <div className='w-full flex items-center justify-between'>
            ...
            <div  className='flex md:hidden mt-2 absolute right-0 mr-8'>
              {!nav ? 
              (<div onClick={navHandler} className='cursor-pointer'><AiOutlineMenu  size="25px"/></div>)
              :
